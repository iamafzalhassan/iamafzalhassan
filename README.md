...
<!-- Add: [![LinkedIn](https://img.shields.io/badge/LinkedIn-Afzal_Hassan-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/<your-handle>) -->
...
|---|---|---|
| **[Evenly](https://github.com/iamafzalhassan/Evenly)**<br/><sub>Kotlin Multiplatform · Compose Multiplatform · Supabase</sub> | Shared expense tracker for Android and iOS from a single codebase | Offline-first sync engine with **sequence-based pulls** (immune to clock skew), per-row last-writer-wins and tombstone deletes · Postgres **row-level security** · exact integer currency conversion and largest-remainder splits · settles any group in **at most n − 1 payments** · native biometric lock on both platforms |
| **[biller](https://github.com/iamafzalhassan/biller)**<br/><sub>Flutter · Riverpod · SQLite</sub> | Offline point-of-sale billing app running at a live wholesale counter | Pixel-aligned multi-page A5 PDF receipts · custom **ESC/POS** Bluetooth thermal printing · offline device activation with **Ed25519** signatures · salted, iterated SHA-256 PIN with constant-time compare · crash-safe draft recovery |
| **[Debitter](https://github.com/iamafzalhassan/Debitter)**<br/><sub>Kotlin · Jetpack Compose · MVVM</sub> | Native Android app replacing a manual Word workflow for a customs clearing business | **Custom PDF layout engine** on `Canvas` with no third-party library · decimal-aligned tabular figures and automatic page overflow · `BigDecimal` money with one formatter shared by UI and PDF · non-destructive SQLite migrations |
| **[muwaqqit](https://github.com/iamafzalhassan/muwaqqit)**<br/><sub>Flutter · BLoC · Clean Architecture</sub> | Full-screen prayer-times board for masjid displays | Fully offline astronomical calculation · live Azan and Iqamah countdowns with day rollover · GPS with graceful fallback · layouts that scale from tablet to TV |
| **[rawg](https://github.com/iamafzalhassan/rawg)**<br/><sub>Flutter · BLoC · Supabase</sub> | Game discovery app built on the RAWG API | Offline-first **Hive cache with 24-hour TTL** and automatic network fallback · debounced search with infinite-scroll pagination · Supabase authentication with route protection · OneSignal push with deep links · English and Sinhala localization |

<sub>More: **[mmf](https://github.com/iamafzalhassan/mmf)** (Flutter, Clean Architecture, BLoC, multi-step validated forms)</sub>
